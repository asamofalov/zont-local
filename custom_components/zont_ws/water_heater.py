...
class ZontDhwWaterHeater(ZontObjectCoordinatorEntity, WaterHeaterEntity):
    ...
    async def async_set_temperature(self, **kwargs: Any) -> None:
        ...
        try:
            await async_set_heating_circuit_temperature(
                self._client,
                self._object_id,
                float(temperature),
            )
        ...
        try:
            refreshed = await self.coordinator.async_refresh_object(self._object_id)
        except asyncio.CancelledError:
            raise
            ...
            if not refreshed:
                _LOGGER.debug(
                    "The ZONT controller accepted the target temperature for object "
                    "%s but did not return a usable state",
                    self._object_id,
                )
