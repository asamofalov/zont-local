...
async def test_discovery_returns_all_parseable_protocol_objects() -> None:
    requests = AsyncMock()
    ...
    assert list(objects) == [8196, 9000]
    assert isinstance(objects[8196], ZontDigitalTemperatureSensorData)
    assert isinstance(objects[9000], ZontRadioSensorData)
...
async def test_discovery_fails_when_a_type_list_is_incomplete() -> None:
    ...
    with pytest.raises(ZontObjectDiscoveryError):
        await async_discover_objects_from_requests(requests)
